...
namespace Builders.PDF
{
    ...
    {
        public PrintInvoice(User _user)
        {
            user = _user;
        ...
        public void Print()
        {
            ...
            signature.Date = Invoice.DateInvoice;
            signature.User = user;
            signature.Print();
            ...
            first.Date = Invoice.DateInvoice;
            first.User = user;
            first.Print();
        ...
        private void CreatePage()
        {
            ...
            paragraph.AddFormattedText("FINAL INVOICE", TextFormat.Bold);
            ...
            paragraph.AddFormattedText(Invoice.DateInvoice.ToShortDateString(), TextFormat.Bold);
            paragraph.AddLineBreak();
            paragraph.AddText("Invoice: ");
            paragraph.AddFormattedText(Invoice.NumberInvoice, TextFormat.Bold);
            paragraph.AddLineBreak();
            ...
            paragraph.AddLineBreak();
            paragraph.AddLineBreak();
            //paragraph.AddText("SO/Pick-Up: ");
            //paragraph.AddFormattedText((Invoice.UpNumber) ?? "", TextFormat.Bold);
            //paragraph.AddLineBreak();
            //paragraph.AddText("PO: ");
            //paragraph.AddFormattedText((Invoice.OrderNumber) ?? "", TextFormat.Bold);
            //paragraph.AddLineBreak();
            ...
            foreach (var item in material)
            {
                ...
                {
                    if (!string.IsNullOrWhiteSpace(item.Depth))   // NLКвота зроблена вручну (без генератора)
                    {
                    ...
                    {
                        string[] words = item.Description.Split(' ');     // Вирішуємо проблему, бо з генератора частина данних приходить тільки в назві Квоти.
                                                                          // Тому прийнято рішення парсити опис Квоти і витягувати решту даних. :)
                        int count = 0;
                        int size = 0;
                        string depth = "";
                        decimal qty = 0m;
                        ...
                        foreach (string word in words)
                        {
                            ...
                            count++;
                                                                    ...
                                                                    // NLКвота зроблена через генератор
                        row.Cells[1].AddParagraph(decript ?? "").Format.Alignment = ParagraphAlignment.Left;
